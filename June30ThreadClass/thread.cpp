...
void Thread::start() {
  ...
  if (!detached) {
    ...
    if (status) {
      printError("pthread_attr_setdetachstate failed at", status, __FILE__,
                 __LINE__);
      exit(status);
    }
...
void Thread::printError(char *msg, int status, char *fileName, int lineNumber) {
  cout << msg << " " << fileName << ":" << lineNumber << "-" << strerror(status)
       << endl;
}

void TestClass::print(){
    cout << "Test Class defined in thread.h and function is in thread.cpp" << endl;
}
