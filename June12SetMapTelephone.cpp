...
/**
Telephone Directory maintains a map of name/number pairs
...
vector<string> TelephoneDirectory::find_entries(int number) {
  multimap<int, string>::iterator lower = inverse_database.lower_bound(number);
  multimap<int, string>::iterator upper = inverse_database.upper_bound(number);
  vector<string> result;

  for (multimap<int, string>::iterator pos = lower; pos != upper; pos++)
    result.push_back(pos->second);
  return result;
}

void TelephoneDirectory::print_all() {
  for (map<string, int>::iterator pos = database.begin(); pos != database.end();
       pos++) {
